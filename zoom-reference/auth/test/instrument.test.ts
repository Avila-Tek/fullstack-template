...
import { createSentryOptions } from '../src/instrument.js';
...
describe('createSentryOptions', () => {
	...
	it('returns enabled: false when SENTRY_DSN is not set', () => {
		delete process.env.SENTRY_DSN;
	...
	it('returns enabled: false when SENTRY_DSN is an empty string', () => {
		process.env.SENTRY_DSN = '';
		expect(createSentryOptions().enabled).toBe(false);
	});

	it('returns enabled: false when NODE_ENV is production but SENTRY_DSN is unset', () => {
		// Regression: NODE_ENV no longer gates Sentry — DSN presence does.
