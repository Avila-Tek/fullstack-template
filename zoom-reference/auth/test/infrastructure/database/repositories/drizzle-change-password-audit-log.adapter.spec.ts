...
import { DrizzleChangePasswordAuditLogAdapter } from '../../../../src/infrastructure/database/repositories/drizzle-change-password-audit-log.adapter';
...
describe('DrizzleChangePasswordAuditLogAdapter', () => {
	let adapter: DrizzleChangePasswordAuditLogAdapter;
	...
	beforeEach(() => {
		...
		logger = makeLogger();
		adapter = new DrizzleChangePasswordAuditLogAdapter(
			mocks.db as never,
			logger,
		);
	});
	...
	it('writes expected columns to securityAuditLog', async () => {
		await adapter.log({
			eventType: 'password_change_succeeded',
			userId: 'user-1',
			ipAddress: '1.2.3.4',
			userAgent: 'ua',
		...
		expect(row.eventType).toBe('password_change_succeeded');
	...
	it('defaults optional fields to null when not provided', async () => {
		await adapter.log({ eventType: 'password_change_failed' });
	...
	it('catches DB errors and logs a warning without throwing', async () => {
		...
		adapter = new DrizzleChangePasswordAuditLogAdapter(db as never, logger);

		await expect(
			adapter.log({ eventType: 'password_change_succeeded', userId: 'u1' }),
		).resolves.toBeUndefined();

		expect(logger.warn).toHaveBeenCalledOnce();
		const [ctx, msg] = (logger.warn as ReturnType<typeof vi.fn>).mock.calls[0];
		expect(msg).toBe('change_password_audit_log_failed');
