...
// Spec §10: signed, time-limited token for email-change verification.
// Format: `base64url(JSON({userId,newEmail,timestamp})).<hmac-sha256-hex>`
// The HMAC covers the entire base64url payload, so tampering with any field
// invalidates the signature. The dot separator avoids collisions with email
// special characters (email can legally contain colons in quoted local parts).
