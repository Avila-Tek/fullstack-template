...
export const account = pgTable(
	'account',
	{
		...
		// Null for credential accounts (no provider).
		providerEmail: text('provider_email'),
		// Normalized form of providerEmail (lowercase + Gmail dot/plus stripping etc.
		// per @zoom/utils normalizeEmail). Computed in databaseHooks.account.create.*
		// when providerEmail is present. Null whenever providerEmail is null.
		normalizedProviderEmail: text('normalized_provider_email'),
	...
	(table) => [
		...
		// Non-unique index to support lookups by normalized provider email
		// (e.g. social-account disconnection in change-email after-hook).
		index('idx_account_normalized_provider_email').on(
			table.normalizedProviderEmail,
		),
	],
);
