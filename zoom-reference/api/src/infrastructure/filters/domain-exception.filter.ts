...
export class DomainExceptionFilter implements ExceptionFilter {
	constructor(
		...
		// Per apps/api/CLAUDE.md: "Status ≥ 500 always resolves to
		// INTERNAL_ERROR — never leak raw error details." For 5xx, return a
		// generic code + message so the client cannot enumerate domain
		// failures or learn anything about internal state. The original
		// domain error survives in logs (above) and on the Sentry event tag
		// for incident triage.
