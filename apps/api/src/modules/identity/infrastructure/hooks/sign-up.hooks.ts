...
import { googleCaptchaService } from '../captcha/google-captcha-adapter';
import { auditLogger } from '../shared/logger/audit-logger';
import { recordAuthEvent } from '../shared/metrics/auth-metrics';
import { envTermsVersionService } from '../terms-version/env-terms-version-adapter';
...
// ---------------------------------------------------------------------------
const pendingTermsMap = new Map<string, PendingTermsEntry>();
...
async function verifyTermsVersion(
	...
	const resolvedVersion = await envTermsVersionService.resolveCurrentVersion();
	if (resolvedVersion === null) {
		return { error: jsonResponse(503, { error: 'terms_version_unavailable' }) };
	}
	const submitted =
		typeof body.termsAcceptedVersion === 'string'
			? body.termsAcceptedVersion
			: '';
	if (resolvedVersion !== submitted) {
		return { error: jsonResponse(422, { error: 'terms_version_mismatch' }) };
	}
