...
The following official shadcn-ui components are available. Please check to see if they are installed under `package/ui` before installing them with `npx shadcn@canary add <component-id>`.

Notice that The 'shadcn-ui' package is deprecated. Please use the 'shadcn' package instead. For example:

```bash
npx shadcn@canary add progress --cwd apps/<client/admin>
```

YOU MUST use `npx shadcn@canary` when installing shadcn packages.
