...
npx turbo lint --filter @repo/admin          # fix: npx turbo lint:fix --filter @repo/admin
...
- **React component files** (`.tsx`): `PascalCase` (e.g., `AdminLoginPage.tsx`, `AdminLayout.tsx`)
...
- **`.env.example` must stay in sync**: any time you add, rename, or remove a `process.env.*` reference, update `apps/admin/.env.example` with a matching entry and an explanatory comment. Never leave an env var undocumented.
...
- **Location**: test files live in `src/__tests__/` mirroring the `src/` structure (e.g. `src/features/auth/ui/pages/AdminLoginPage.tsx` → `src/__tests__/features/auth/ui/pages/AdminLoginPage.test.tsx`).
